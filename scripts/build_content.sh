...
if [ ${#STEPS_TO_RUN[@]} -eq 0 ]; then
    STEPS_TO_RUN=(sync_translations build_hugo offload_images find_duplicate_images translate sync_content_attributes sync_translation_urls generate_translation_urls generate_amplify_redirects generate_related_content generate_linkbuilding_keywords extract_automatic_links precompute_linkbuilding apply_linkbuilding preprocess_images)
