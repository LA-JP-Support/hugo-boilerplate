...
description: 精度測定は、AIや機械学習モデルが正しく予測・分類できている割合を数値化する指標。モデルの性能を評価するために欠かせません。
keywords:
- 精度
- 機械学習
- 性能指標
...
**精度（Accuracy）は、AIモデルが正しく予測した割合を示す指標です。** たとえば、100個のメールのうち96個を正しくスパムか正当かに分類できたなら、精度は96%です。機械学習やAIシステムの性能を評価する最も基本的で直感的な指標ですが、実は落とし穴があります。99%のメールが正当で1%がスパムのデータセットで、「すべて正当」と予測するだけで99%の精度を達成してしまい、実際にはスパムを全く識別していない、といった事態が生じます。

> **ひとことで言うと：** 「テストで何問正解したか」を数値で示すようなもの。でも0点でも正解率100%と言えてしまう場合もあります。

**ポイントまとめ：**

- **何をするものか：** AIモデルの正解率を測定する
- **なぜ必要か：** モデルの良し悪しを数字で判定し、改善の効果を測るため
- **誰が使うか：** AI開発企業、データ科学者、機械学習エンジニア
...
具体例として、1000件のメール分類の場合：正当メール920件を正しく判定、スパムメール45件を正しく判定、正当メール10件を誤ってスパムと判定、スパムメール25件を見逃したとします。精度 = （920 + 45） / 1000 = 96.5%となります。

## 目安・ベンチマーク

精度の目安はアプリケーションの種類によって大きく異なります。スパムメール判定なら85～95%で十分ですが、医療診断AIは95%以上が必要で、自動運転車は99.9%以上の精度が求められます。ただし、精度だけでは不十分な場合が多いため、並行して以下を監視します。適合率（正と予測したもののうち本当に正の割合）、再現率（実際の正のうち正しく予測できた割合）、F1スコア（適合率と再現率の調和平均）。特に医療診断のように見逃しが致命的な分野では、精度よりも再現率を重視します。

## 精度を補う重要な指標

精度が高くても、特定のエラーが多い場合があります。たとえば、スパム検出で99.5%の精度を達成したモデルが、実は正当メール1000件中9950件は「スパムでない」と正しく判定するが、スパム50件を全く識別していない場合、精度だけ見ると優秀に見えても目的を果たしていません。このため、以下の指標を並行監視します。適合率は「スパムと判定したもののうち本当にスパムはどれか」、再現率は「すべてのスパムのうち正しく検出できたのはどれか」を測ります。F1スコアはこの両者のバランスを取る指標です。

## なぜ精度だけでは不十分か

不均衡なデータセットにおいて、精度は誤解を招きやすいという根本的な問題があります。人口の0.1%しかない疾患の検出で「全員健康」と予測するモデルは99.9%の精度を示しながら、実際には全く機能していません。このパラドックスを避けるため、データ品質の改善、継続的な監視、ドメインに適した指標の選択が重要です。さらに、モデルがトレーニングデータを丸暗記してしまう過学習に陥ると、トレーニング時の精度は高いが本番データで精度が落ちるという問題も生じます。

## 関連用語

- **[混同行列](Confusion-Matrix.md)** — 予測結果を整理した表。精度計算の基になります。
- **[F1スコア](F1-Score.md)** — 適合率と再現率のバランスを測る指標です。
- **[機械学習](Machine-Learning.md)** — 精度測定の対象となる技術分野です。
- **[データドリフト](Data-Drift.md)** — 時間とともにモデルの精度が低下する現象を検出します。

## よくある質問

**Q: 精度だけで十分でないのはなぜ？**
A: 不均衡なデータ（スパムが1%のメール分類など）では、精度が高くても実用性が低い場合があります。適合率や再現率も合わせて確認しましょう。

**Q: 医療診断では精度より何を重視すべき？**
A: 病気を見逃す（偽陰性）は致命的なため、再現率が最優先です。精度が高くても見逃しが多いモデルは危険です。

**Q: モデルの精度はどのくらい監視すべき？**
A: 重要なシステムは毎日、標準的なシステムは週1回、低リスクなら月1回が目安です。精度が急激に低下したら再訓練が必要です。
