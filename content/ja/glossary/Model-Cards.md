...
description: モデルカードは、機械学習モデルのための標準化されたドキュメントであり、アーキテクチャ、想定される用途、パフォーマンス、制限事項、トレーニングデータ、倫理的考慮事項などを詳述し、透明性と説明責任を確保します。
keywords:
- モデルカード
- 機械学習
- AI倫理
...
モデルカードは、標準化されたモデルドキュメンテーションの欠如と機械学習システムの不透明性に対処するため、2018年にGoogleの研究者によって初めて提案されました。Mitchell氏らによる基礎論文["Model Cards for Model Reporting"](https://arxiv.org/abs/1810.03993)は、特に医療、金融、法執行などの機密性の高い領域において、モデルの特性と社会的影響を透明に伝える必要性を強調しています。

### 動機

- **透明性:** モデルが何をするか、どのようにトレーニングされたか、既知の制限事項を明確に表現する。
- **説明責任:** ステークホルダーがモデルの動作を理解し監査できるようにする。これは規制遵守と公共の信頼にとってますます重要になっている。
- **公平性:** バイアスと不均等な影響の明示的な報告と分析を促進する。

### 採用状況

モデルカードはAIコミュニティにおけるベストプラクティスとなっています:
- **Hugging Face:** [Model Hub](https://huggingface.co/models)の数千のモデルにモデルカードが含まれている。
- **Meta、OpenAI、Google:** 主要なAIラボが主力モデルのモデルカードを公開([Meta Llama](https://github.com/facebookresearch/llama/blob/main/MODEL_CARD.md)、[OpenAI GPT-3](https://github.com/openai/gpt-3/blob/master/model-card.md)、[Google Face Detection](https://modelcards.withgoogle.com/face-detection))。
- **IBM:** [AI FactSheets](https://aifs360.mybluemix.net/introduction)は、エンタープライズガバナンスのためにモデルカードの概念を拡張している。

## モデルカードとは何か?

モデルカードは、機械学習モデルの主要な技術的・倫理的属性を記述する簡潔かつ包括的な文書です。通常、以下の内容が含まれます:
...
### 3. バイアスと公平性の緩和
モデルカードは、人口統計グループ全体でのモデルパフォーマンスの定量的・定性的分析を要求し、害を特定し軽減するのに役立ちます([arXiv:1810.03993](https://arxiv.org/abs/1810.03993))。
...
### 6. ステークホルダーコミュニケーション
非技術者と技術者が共通の理解を得ることができ、これは責任ある展開と公共の信頼にとって重要です。

## モデルカードの主要セクション

テンプレートは様々ですが、包括的なモデルカードには通常以下のセクションが含まれます:
...
| ビジネス詳細 | 商業およびサポート情報 | ライセンス、連絡先、監視ガイダンス |
...
- [IBM AI FactSheets](https://aifs360.mybluemix.net/introduction) — 責任あるAIのためのエンタープライズドキュメンテーション。
...
- 法的・規制的枠組み(例:GDPR、EU AI Act)への準拠を確認する。
- 社会的影響と公平性を評価する。
...
- プロジェクト全体で一貫したドキュメンテーションを確保する。

### 3. 可能な限り自動化

- カード生成をCI/CDまたは[MLOps](/en/glossary/mlops/)パイプラインに統合する。
...
- モデルカードを生きた文書として扱う。
- 再トレーニングまたは主要な評価ごとに更新する。

### 6. アクセスの集中化

- 検索可能なレジストリを提供する。
- 機密または独自のモデルのアクセスを制御する。

### 7. 多様なステークホルダーの関与

- 技術、ビジネス、法務、倫理チームを関与させる。
...
*この用語集ページは、実務者、ビジネスリーダー、政策立案者、そして機械学習モデルが[透明性](/en/glossary/transparency/)、倫理的厳格さ、実用的な有効性をもって文書化、評価、展開されることを確保したいすべての人を対象としています。*
...
**Markdown形式の用語集、完全な例とステークホルダー固有の詳細を含めると5,000語以上になります。最新のテンプレートと例については、常に上記にリンクされている公式ドキュメンテーションとリポジトリを参照してください。**