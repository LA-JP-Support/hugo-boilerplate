...
| 用語                    | 定義                                                                                                                         |
|-------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| **Text-to-Speech (TTS)**| 書かれたテキストを計算モデルを使用して音声オーディオに変換します。                                                                 |
| **音声合成**    | 人間の音声を人工的に生成すること。                                                                                             |
| **ノード/ブロック**          | 特定の機能(例:TTS変換)のためのモジュール式ワークフローユニット。                                                               |
| **ニューラルTTS**          | ディープニューラルネットワークによって駆動されるTTS、自然なプロソディと品質を実現します。                                                         |
| **SSML**                | 発音、強調、ピッチなどを制御するXMLベースのマークアップ。([SSMLドキュメント](https://www.w3.org/TR/speech-synthesis/))           |
| **プロソディ**             | 音声におけるリズム、強勢、イントネーションのパターン。                                                                                 |
| **ボコーダー**             | 音響表現を音声波形に変換するアルゴリズム。                                                                  |
| **キャッシング(TTSにおける)**    | 繰り返し発話のために生成されたオーディオの保存。                                                                                |
| **音声モデル**         | 特定の音声を表す事前トレーニング済みAIモデル。                                                                                |
| **音声サービス**      | 音声合成のためのクラウド/ローカルAPI(例:Azure、Google、OpenAI、ElevenLabs、[Advanced TTS MCP](https://github.com/samihalawa/advanced-tts-mcp))。|
| **合成音声**  | TTSシステムによって生成された出力オーディオ。                                                                                            |
| **言語分析** | 発音、強勢、言語特性を決定する段階。                                                                        |
| **カスタム音声**        | 特定の話者またはブランド向けにトレーニングされた音声モデル。([Azure Custom Neural Voice](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/custom-neural-voice))|
| **リアルタイム合成** | 低遅延でのオンデマンド音声生成。                                                                                      |
