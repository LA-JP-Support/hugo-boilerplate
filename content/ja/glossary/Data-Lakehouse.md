...
description: データレイクの柔軟性とデータウェアハウスのパフォーマンスを統合したアーキテクチャです。
keywords:
- データレイクハウス
- データアーキテクチャ
- レイク
...
aliases:
- /ja/glossary/Data-Lakehouse/
term: データレイクハウス
---

## データレイクハウスとは?

**データレイクハウスは、[データレイク](Data-Lake.md)の柔軟性と[データウェアハウス](Data-Warehouse.md)のパフォーマンスを融合させたデータアーキテクチャです。** 一つのプラットフォームで、非構造化データから構造化データまで保存でき、かつ高速にクエリできるようになりました。従来は「データレイク」と「データウェアハウス」を二つ運用していた企業が、一つのシステムに統一できるようになったのです。

> **ひとことで言うと：** 「安く大量のデータを保存できる倉庫」と「使いやすく高速に分析できる机」が一つになった、良いとこどりの施設です。

**ポイントまとめ：**

- **何をするものか：** あらゆる種類のデータを一元管理し、高速に[分析](Data-Analytics.md)できるシステムです。
- **なぜ必要か：** データ管理システムの複数運用による複雑性と高コストを削減するためです。
- **誰が使うか：** 大規模データを扱う企業、データドリブン企業です。

## なぜ重要か

従来、企業は2つのシステムを運用していました。[データレイク](Data-Lake.md)は安いがカオスになりやすく、[データウェアハウス](Data-Warehouse.md)は高速だが高額です。この二つの間を行き来するのに時間とコストがかかりました。レイクハウスは、[Delta Lake](Delta-Lake.md)などの技術により、安いストレージの上でもウェアハウス品質のパフォーマンスを実現しました。
...
[データガバナンス](Data-Governance.md)は最初から組み込まれており、誰がどのデータにアクセスできるか、どの質問や変更が許可されるか、自動的に管理されます。
...
メリットは、[ストレージコスト](Storage-Cost.md)が従来のウェアハウスの数分の一になることです。複雑なETLパイプラインが不要になり、運用負荷も低下します。

注意点は、セットアップに高度な技術スキルが必要という点です。データの質が悪いと、レイクハウスの恩恵が受けられません。また、[セキュリティ](Data-Security.md)設定を誤ると、大量の機密データが漏出するリスクもあります。

## 関連用語

- **[Delta Lake](Delta-Lake.md)** — レイクハウス実装に使われるオープンソースストレージ層。トランザクション機能を追加します。
- **[データレイク](Data-Lake.md)** — 無加工のデータを大量保存する仕組み。レイクハウスはここから発展しました。
- **[データウェアハウス](Data-Warehouse.md)** — 整理されたデータを高速に分析する仕組み。レイクハウスが目指すパフォーマンスを備えています。
- **[Data Pipeline](Data-Pipeline.md)** — データを取り込み・変換・保存する仕組み。レイクハウス内で実行されます。
- **[データガバナンス](Data-Governance.md)** — データの質と安全性を管理する仕組み。レイクハウスに必須です。

## よくある質問

**Q: 普通のデータウェアハウスではだめ?**
A: 非構造化データ（画像、テキスト、ログ）を大量に保存したい場合、ウェアハウスは向きません。また、ウェアハウスはかなり高額です。レイクハウスは「全部入り」で安いことが強みです。

**Q: どのくらいのデータ量から必要?**
A: テラバイト単位以上のデータを扱い、複数の分析チームがいる企業が対象です。100ギガバイト程度なら、レイクハウス導入のメリットは限定的です。

**Q: Delta LakeとApache Icebergの違いは?**
A: どちらもレイクハウス実装に使われる技術ですが、Deltaはシングルテーブル、Icebergはマルチテーブル対応が強みです。ユースケースで使い分けます。
