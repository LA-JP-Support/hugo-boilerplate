...
description: 顧客満足度スコア(CSAT)は、特定の体験やサポート対応への満足度を瞬間的に測定する指標。計算方法・ベンチマーク・改善施策を解説します。
keywords:
- 顧客満足度スコア
- CSAT
- 顧客体験
...
顧客満足度スコア(CSAT:Customer Satisfaction Score)は、特定のサービス対応・商品購入・サポート経験など、単一の体験に対する顧客の満足度を測定する指標です。「この対応にどれくらい満足していますか?」という直接的な質問に基づき、顧客満足度をパーセンテージで数値化します。

> **ひとことで言うと：** 「今この瞬間、顧客はどう感じているのか」をシンプルに数値化するツールです。

**ポイントまとめ：**
- **何をするものか：** 購入直後・サポート完了直後など接点直後に、顧客の満足度を即座に把握します。
- **なぜ必要か：** 問題が大きくなる前に顧客の不満に気づき、個別対応・サービス改善につなげられます。
- **誰が使うか：** コンタクトセンター、eコマース、SaaS、小売など顧客接点を持つ全業界が活用します。
...
例えば100人の顧客に満足度を5段階評価(1〜5)で聞き、80人が「4」または「5」と答えた場合、CSAT=80%となります。満足の定義(4以上か5のみか)は業界・企業の基準により異なります。

**業界別ベンチマーク目安：**
- eコマース:75〜85%
- カスタマーサポート:80%〜90%
- SaaS/サブスクリプション:70%〜85%
- 金融機関:80%以上が好ましい

## 計測のタイミングと方法

CSATは体験がまだ記憶に新しい「直後」に測定することが最重要です。購入完了直後、チャットボット対応終了直後、電話サポート終了直後、納品確認直後など、その時間帯の適切なタイミングに質問を配信します。

通常は1〜3問の簡潔な形式で、スマイリーフェイス・星評価・数値スケール等の視覚的選択肢が用いられます。モバイル普及により、アプリ内ポップアップ・SMS・メール等、顧客が最も使用しているチャネルでの配信が重要です。

## 目安・ベンチマーク

CSATスコアの評価は業界・製品・競合により大きく異なります。一般的には:

- **85%以上：** 優秀。顧客離脱リスク低く、口コミによる新規紹介期待大。
- **75〜84%：** 合格。業界平均レベルだが改善の余地あり。
- **65〜74%：** 要注意。競合との比較で劣位の可能性。
- **65%未満：** 危機的。即座の改善施策必須。

ただしCSATは「その瞬間の満足」を測るため、全体的なロイヤルティ(NPS)や維持意向とは異なることに注意が必要です。

## 改善施策

CSATスコアが低い場合、以下の施策が効果的です：

**スピード向上** - 対応時間短縮・初回対応解決率向上で待ち時間ストレス削減。
**ナレッジ向上** - 担当者の製品理解・トラブルシューティング知識向上で正確性向上。
**コミュニケーション品質** - 傾聴・共感・明確な説明で顧客の心情的満足を高める。
**フィードバックループ** - 低評価コメントの詳細分析と具体的な改善実装を顧客に返告。

## 関連用語

- **[顧客維持](Customer-Retention.md)** — 満足度をCSATで測定し、低い顧客に対して維持施策を実施します。
- **NPS(ネットプロモータースコア)** — 「友人に推奨するか」を測る、CSATより長期的なロイヤルティ指標です。
- **[カスタマーエフォートスコア](Customer-Retention.md)** — 「問題解決がどれくらい簡単だったか」を測る補完指標です。
- **[顧客体験](Customer-Relationship-Building.md)** — CSATは体験品質を数値化するツールです。
- **コンタクトセンター運営** — CSAT測定とその結果フィードバックは運営の基本プロセスです。

## よくある質問

**Q: CSATとNPSの違いは？**
A: CSATは「その瞬間の満足」、NPSは「長期的なロイヤルティ・推奨意向」を測ります。CSATが高い対応でもNPSが低い(推奨しない)ことはあります。両者を併用し、短期と長期の双方を把握するのがベストプラクティスです。

**Q: 回答率が低い場合どうするか？**
A: 質問数を1問に限定・スマイリーフェイス等直感的UI・インセンティブ(抽選)・対応直後の即座配信が有効です。ただし回答者が「満足/不満が強い人」に偏るバイアスを認識し、平均値だけでなく分布を見ることが重要です。

**Q: 低評価へのフォローはどうするべきか？**
A: 「ご指摘ありがとうございます」と誠実に受け止め、具体的な改善内容を伝え、改善後に再度アプローチするのが信頼再構築の定石です。その顧客だけでなく、低評価の共通パターン分析から組織的改善につなげることが重要です。
## 顧客満足度スコア(CSAT)とは?
