...
- **モノリシックアプリケーション:**  
  単一の不可分なコードベースとして構築され、一つのユニットとしてデプロイされるソフトウェアアプリケーション。

- **密結合:**  
  モジュール/コンポーネントが互いの内部詳細に高度に依存しているシステム。

- **単一デプロイメントユニット:**  
  アプリケーション全体が単一のアーティファクトとしてビルド、テスト、リリースされる。

- **垂直スケーリング:**  
  アプリケーションを実行しているサーバーのリソース(CPU、RAM)を増やすこと—スケールアップ。

- **水平スケーリング:**  
  ロードバランサーの背後でアプリケーションの複数のコピーを実行すること—スケールアウト。

- **マイクロサービス:**  
  アプリケーションを小さく、独立した、疎結合のサービスに分割するアーキテクチャアプローチで、それぞれが独自のライフサイクルを持つ。

- **継続的インテグレーション/継続的デリバリー(CI/CD):**  
  頻繁で自動化されたビルド、テスト、デプロイメントのための実践とツール。
