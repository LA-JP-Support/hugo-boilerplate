...
description: マルチテナンシーは、1つのアプリケーションで複数の顧客を効率的にサポートする仕組みです。コストを削減しながら、顧客データを安全に分離して保ちます。
keywords:
- マルチテナンシー
- SaaS
- クラウドコンピューティング
...
aliases:
- "/ja/glossary/Multi-Tenancy/"
---

## マルチテナンシーとは

**マルチテナンシーは、複数の顧客（テナント）が1つのアプリケーションとデータベースを共有しながら、互いのデータは完全に分離されている仕組みです。** クラウドサービスやSaaS（Software-as-a-Service）の標準的な方式です。例えば、Salesforceは数百万の企業が同じプラットフォームを使用していながら、各企業のデータはテナントIDで論理的に分離され、絶対に他社には見えません。

> **ひとことで言うと：** アパートメントビルのようなもの。複数世帯が1つの建物を共有し、建物の電気・水道代を折半しながら、各世帯は独立したプライベート空間を持ちます。

**ポイントまとめ：**

- **何をするものか：** 複数の顧客に対して、共有インフラから独立したサービスを提供すること
- **なぜ必要か：** コストと運用効率を大幅に削減できるから
- **誰が使うか：** SaaS企業、クラウドサービスプロバイダー、大規模プラットフォーム

## なぜ重要か

マルチテナンシーはビジネス効率の基盤です。顧客ごとに別々のアプリケーションとサーバーを用意していたら、Salesforceのような1000万社以上の顧客を支えられません。マルチテナンシーにより、インフラコストを顧客数で分散でき、小規模スタートアップから大規模企業まで、柔軟な価格帯でサービスを提供できます。同時に、すべての顧客が同じバージョンを使うため、更新配布が簡単で、全体のセキュリティも統一的に管理できます。

## 仕組みをわかりやすく解説

マルチテナンシーのデータ分離には3つのアプローチがあります。**テナントID方式**が最もシンプルで、全ての行に顧客を示すIDを付与し、クエリ時に自動フィルタリングします。**スキーマ分離方式**は1つのデータベース内に顧客ごとのスキーマを作成し、より強い分離を実現します。**データベース分離方式**は顧客ごとに専用データベースを用意し、最も安全ですが、運用コストが増加します。

アプリケーション層では、ユーザーがログインするとシステムはテナントを特定し、その後のすべてのクエリに自動的にテナントIDフィルターを適用します。これにより、開発者は顧客A向けと顧客B向けのコードを分けて書く必要がなく、同じコードが全てのテナントに対応します。

## 実際の活用シーン

**Salesforce CRM**

各企業がカスタムフィールドや設定を独立して構成できながら、全て同じプラットフォーム上で動作。更新は一度で全企業に配信されます。

**クラウドストレージ（Google Drive）**

数十億ユーザーが同じインフラを共有しながら、自分のファイルだけアクセス可能。コスト効率と利便性を両立させています。

**プロジェクト管理ツール（Asana、Notion）**

複数企業がプロジェクトを管理しながら、組織ごとのデータ完全分離を実現。セキュリティと効率を両立させています。

## メリットと注意点

**メリット：コスト削減と効率**

インフラを共有するため顧客あたりのコストが低く、スケールメリットが働きます。更新も1回で全顧客対応できます。

**注意点：セキュリティリスク**

一度分離がバグで崩れると、顧客データが露出する可能性があります。また「ノイジーネイバー」問題で、一部の重いテナントが他テナントのパフォーマンスを低下させることがあります。

## 関連用語

- **[SaaS](SaaS.md)** — マルチテナンシーで実現される提供モデル
- **[クラウドコンピューティング](Cloud-Computing.md)** — マルチテナンシーの実行基盤
- **[データセキュリティ](Data-Security.md)** — マルチテナンシーの最大の課題
- **[スケーラビリティ](Scalability.md)** — マルチテナンシーが解決する課題

## よくある質問

**Q: マルチテナンシーで本当にデータは分離されている？**
A: はい。適切に実装されていれば絶対分離されます。ただし、バグ（SQLインジェクション等）があるとその限りではないため、セキュリティ監査が重要です。

**Q: ノイジーネイバー問題は防げる？**
A: はい。リソースクォーターやアクティブなモニタリングで対処できます。ただし、完全には防げず、高い要求を持つテナントは物理分離を求めることもあります。
## マルチテナンシーとは?
