...
| 用語 | 定義 |
|------|------|
| Sharding | 単一の論理データセットを複数のデータベース(シャード)に分割し、各シャードがデータのサブセットを保持 |
| シャードキー | データレコードをシャードに割り当てるために使用される属性 |
| ハッシュベースSharding | シャードキーにハッシュ関数を使用するSharding戦略 |
| 範囲ベースSharding | シャードキーの値範囲をシャードに割り当てることによるSharding |
| ディレクトリベース | ルックアップテーブルを使用してキーまたは範囲を特定のシャードにマッピングするSharding |
| 水平スケーリング | より多くのサーバー(ノード)を追加することで容量を増やす |
| 垂直スケーリング | 単一サーバーのハードウェアをアップグレードすることで容量を増やす |
| ホットスポット | 不均衡なトラフィックまたはデータを受け取り、ボトルネックになるシャード |
| シェアードナッシング | 各ノード/シャードが独立して動作し、リソースを共有しないアーキテクチャ |
| クロスシャードクエリ | 複数のシャードへのアクセスを必要とするクエリ |
| リシャーディング | シャードの追加/削除または歪みの修正時にデータを再分散するプロセス |
