...
description: モデル窃取は、AIモデルにクエリを送信し、その応答を利用して機能を複製する攻撃です。知的財産を脅かし、さらなる攻撃の足がかりになります。
keywords:
- モデル窃取
- モデル抽出
- 機械学習セキュリティ
...
**モデル窃取は、攻撃者がAPIやクエリを通じてAIモデルに系統的にアクセスし、その応答を使ってモデルの機能を複製する攻撃です。** 訓練データやモデル内部の詳細にアクセスせずに、元のモデルの動作と予測を再現できます。これは企業が開発に多大なリソースを投じたモデルの知的財産を効果的に盗むもので、ビジネスに深刻な影響を与えます。

> **ひとことで言うと：** あるお店の営業秘密のレシピを、何度も購入して味を調べることで再現するようなもの。完全な資料は盗まないけれど、試行錯誤で秘密を明かしてしまう状況です。

**ポイントまとめ：**

- **何をするものか：** APIを通じた何度ものクエリでモデルの機能を複製すること
- **なぜ必要か：** 攻撃者にとって高価な訓練を避け、知的財産を獲得する手段になる
- **誰が使うか：** 競合他社のシステムを分析したい攻撃者や、モデルの弱点を調べる研究者

## なぜ重要か

モデル窃取は3つの観点で危険です。まず、企業が[機械学習](LLM.md)モデルに投じた開発投資を損なわせます。次に、盗まれたモデルを使い、敵対的サンプルやモデル反転などのより高度な攻撃の足がかりになります。最後に、盗まれたモデルは永続的に存在し、新しい脆弱性が発見されても修正できず、長期的なリスクを生みます。

## 仕組みをわかりやすく解説

モデル窃取は単純ながら強力です。攻撃者はまず、ターゲットモデルへのアクセス方法を決めます。クエリ予算（実行可能な質問数）を考慮しながら、シンプルなランダム入力から始めるか、より効率的な戦略的サンプリングを使うか選択します。

次に、答え集めフェーズです。モデルが返す全ての確率分布や信頼スコアを記録し、入出力ペアのデータセットを構築します。このデータの質が、複製モデルの精度を左右します。

最後に、収集したデータを使って代替モデルを訓練します。元のアーキテクチャと合わせる必要はなく、同じ動作をすることが目標です。訓練後、テストして一致度を測定し、必要に応じて反復を重ねます。

## 実際の活用シーン

**商用ML APIの盗聴**

クラウドで公開されている画像認識や自然言語処理のAPIに対して、攻撃者が定期的にクエリを送り、応答パターンを学習。開発コストをかけずに競合のモデルを構築します。

**セキュリティシステムの回避**

[多要素認証](Multi-Factor-Authentication--MFA-.md)やスパム検出モデルを複製することで、事前に回避戦略をテストできます。検出されるリスクなく、本当の環境で攻撃を精密化できます。

**プライバシー侵害**

盗まれた顔認識モデルを使って、個人の顔データを復元するモデル反転攻撃が可能になります。盗用元の企業ではなく、攻撃者が自由に実験できるため、プライバシー侵害の規模が拡大します。

## メリットと注意点

**防御側の注意点**

組織が講じるべき対策の有効性には差があります。レート制限（1時間あたりのクエリ数を制限）は有効ですが、複雑さを増すだけで完全には防げません。確度スコアを返さない、回答にノイズを加えるといった方法も、ユーザー体験を損なう可能性があります。

**攻撃者の工夫**

より効率的な抽出手法が研究されており、必要なクエリ数が減少傾向にあります。一方で、完全に防ぐことは困難で、セキュリティと利便性のバランス取りが永遠の課題です。

## 関連用語

- **[大規模言語モデル](LLM.md)** — モデル窃取の対象になることが多い、テキスト生成の基盤技術
- **[プロンプト・エンジニアリング](Prompt-Engineering.md)** — モデルに効果的に質問する方法を理解することで、盗聴効率が上がる可能性
- **[敵対的サンプル](Adversarial-Examples.md)** — 盗まれたモデルを使ってこの攻撃を開発することが多い
- **[アクセス制御](Access-Control.md)** — モデル窃取を防ぐための基本的な防御策

## よくある質問

**Q: パスワードが盗まれるのとモデル盗聴は同じ？**
A: 異なります。パスワード流出は1回で済みますが、モデル盗聴は継続的なアクセスが必要です。ただし盗まれたモデルは永続的に悪用され、パスワード流出より長期的な被害が大きいこともあります。

**Q: 小規模な企業でも対策が必要？**
A: はい。大企業だけが狙われるわけではなく、ニッチな領域で優れたモデルを持つ企業は価値があります。基本的なレート制限と監視は投資対効果が高いです。

**Q: 完全に防ぐことは可能？**
A: 難しいです。公開APIなら完全には防げませんが、複数の層を重ねることで、攻撃のコストと時間を大幅に増やせます。
