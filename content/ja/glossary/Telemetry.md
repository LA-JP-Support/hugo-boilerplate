...
- **Application Performance Monitoring(APM):** アプリケーションの健全性を追跡および最適化するためのツールと技術。
- **オブザーバビリティ:** テレメトリ出力からシステムの内部状態を理解する能力。
- **データレイク:** 生データと構造化テレメトリデータの集中ストレージ。
- **インストルメンテーション:** テレメトリデータ収集を可能にするためにコードまたはエージェントを埋め込むこと。
- **サンプリング:** ボリューム/コストを削減するためにテレメトリデータのサブセットを選択すること。
- **データ保持ポリシー:** テレメトリデータが保存される期間に関するルール。
