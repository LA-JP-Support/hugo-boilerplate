...
- Solicitudes de búsqueda se enrutan desde el cliente → proxy de acceso → nodos de streaming/consulta → resultados agregados y devueltos.
- Inserciones pasan por nodos de streaming, se escriben en WAL, se convierten en segmentos sellados, se indexan y almacenan.

**En profundidad:**  
- [Documentación de Arquitectura Milvus](https://milvus.io/docs/architecture_overview.md)
...
**Conceptos Clave:**
- *Estructuras de Datos:* IVF (clustering/bucketing), HNSW (basado en grafos), Flat (fuerza bruta).
...
- Índices basados en grafos (HNSW) superan a IVF en consultas de bajo k y alta recuperación.
...
Milvus puede servir como almacén vectorial para embeddings generados por modelos de OpenAI, facilitando búsquedas semánticas y RAG.  
...
**Ventajas Distintivas de Milvus:**
- Gran diversidad de índices (HNSW, IVF, DiskANN, Annoy, Cagra)
...
**Ejemplo en Python usando PyMilvus:**
