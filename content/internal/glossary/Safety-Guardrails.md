...
type = "glossary"
draft = false
url = "/internal/glossary/Safety-Guardrails/"
...
**A. Input Guardrails**Validate, sanitize, or reject user prompts and API calls before reaching the model.
...
**B. Output Guardrails**Analyze, filter, or redact model responses before delivery to users.
...
**C. Behavioral Guardrails**Monitor and restrict ongoing AI actions and multi-step agentic workflows.
...
**D. Policy-Based Guardrails**Declarative rules for allowed/denied actions, topics, or content.
...
**E. ML-Based Guardrails**Classifiers and anomaly detectors flag unsafe or out-of-distribution behavior.
...
**F. Ethical and Security Guardrails**Controls to enforce fairness, [transparency](/en/glossary/transparency/), and privacy.
...
**Healthcare:**- Guardrails prevent AI from dispensing direct medical advice.
...
**Finance:**- Guardrails block unauthorized investment recommendations.
- Monitor for leaks of insider information.
- Ensure compliance with SOX and financial data regulations.

**Retail:**- Filter customer PII in support chats.
- Prevent price discrimination.
- Align outputs with brand guidelines.

**SaaS/Technology:**- Prevent confidential code leakage through code generation tools.
...
**Workflow Example:**User → API Gateway (input guardrails) → AI Model (guarded) → Output Filter (output guardrails) → End User.  
