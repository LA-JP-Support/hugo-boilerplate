...
**Technical note:**Continuous and logit scores can be converted using sigmoid or softmax functions, but precision may be affected by floating point limits ([Mindee](https://www.mindee.com/blog/how-use-confidence-scores-ml-models)).
...
1. **Analyze Data Distribution**- Visualize model confidence scores (e.g., histogram of outputs).
   - Identify natural cutoffs or clusters.

2. **Establish Initial Threshold**- Start with a standard (e.g., 0.5 for binary classification).
   - For high-risk domains, start higher (e.g., 0.9).

3. **Test and Iterate**- Evaluate [precision and recall](/en/glossary/precision-and-recall/) at various thresholds.
...
4. **Monitor and Adapt**- Continuously monitor model performance.
...
**Key insight:**- **Raising the threshold**reduces false positives (higher precision) but increases false negatives (lower recall).
