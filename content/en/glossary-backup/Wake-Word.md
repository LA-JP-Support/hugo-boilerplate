...
**Authoritative definition:**> “A word or words that you say in order to make an electronic device, or a feature on a device, ready to work.”  
> — [Cambridge Dictionary](https://dictionary.cambridge.org/us/dictionary/english/wake-word)

**Industry perspective:**Wake word detection is the invisible foundation that powers modern voice AI, making voice interfaces intuitive, seamless, and always available. It is used in smart speakers, mobile devices, cars, appliances, and more ([Picovoice Complete Guide](https://picovoice.ai/blog/complete-guide-to-wake-word/)).

### Synonyms and Related Terms

- **Hotword**- **Trigger Word**- **Wake Phrase**- **Activation Phrase**- **Keyword Spotting**- **Wake-up Word (WuW)**- **Voice Trigger**These terms are interchangeable in technical and product documentation, reflecting the core functionality: monitoring audio for a predetermined activating phrase ([SoundHound](https://www.soundhound.com/voice-ai-blog/what-you-need-to-know-about-wake-word-detection/)).
...
**Pro Tip:**On-device wake word detection conserves energy and maximizes privacy by ensuring that full audio processing only occurs when necessary.

### Examples of Wake Words

**Consumer Voice Assistants:**- “Hey Siri” (Apple)
- “Alexa” (Amazon)
...
**Automotive:**- “Hey Mercedes”, “Hey BMW”, “Hey Porsche”, “OK Honda”, “Hello Kia”

**Home Electronics:**- “Hi LG”, “OK LG”, “Hello Lloyd”

**Custom/Branded:**- “Hey Pandora”, “Hey SoundHound”, “Hey Mycroft”
...
1. **Audio Capture:**The device streams audio from its microphone continuously.

2. **Feature Extraction:**Audio is converted into features such as Mel Frequency Cepstral Coefficients (MFCCs) or mel spectrograms, which efficiently represent speech patterns.

3. **Neural Network Processing:**Deep neural networks (DNNs) process these features to identify the unique acoustic signature of the wake word.

4. **Detection Decision:**The model outputs a confidence score indicating the likelihood that the wake word was spoken.

5. **Activation:**If the score exceeds a threshold, the system activates and transitions to full command processing.

**Model Training Process:**- Collect recordings from hundreds of speakers with varied accents and in different noise conditions.
...
**Efficiency:**Modern wake word engines, such as [Porcupine](https://picovoice.ai/platform/porcupine/) and [Sensory TrulyHandsfree](https://www.sensory.com/wake-word/), are optimized for low [latency](/en/glossary/latency/) and minimal CPU/memory usage, making them suitable for embedded and battery-powered devices.

**On-device vs Cloud:**On-device detection is preferred for privacy, latency, and reliability reasons. Only the audio after the wake word is sent to cloud servers if needed ([Picovoice](https://picovoice.ai/blog/complete-guide-to-wake-word/)).
...
- **Wake Word Detection:**A lightweight binary classifier. It only outputs “yes” or “no” for the presence of a specific phrase.
- **Automatic Speech Recognition (ASR):**A complex system that transcribes full speech into text, regardless of content.

**Why not use ASR for wake words?**- **Resource Intensive:**ASR consumes more CPU/memory.
...
**Best practice:**Wake words are superior for accessibility, natural interaction, and situations where hands-free operation is critical (e.g., driving) ([SoundHound](https://www.soundhound.com/voice-ai-blog/what-you-need-to-know-about-wake-word-detection/)).

## Selection and Design of Wake Words

### Best Practices for Choosing a Wake Word

**Key Design Criteria:**- **Length:**2–4 syllables; balances uniqueness and usability.
...
**Avoid:**Short, generic, or commonly used terms (e.g., “Hi,” “OK”) which can cause frequent false activations.

**Example:**- “Hey Siri” (distinct, 2+2 syllables)
...
**Tip:**Involve local testers and linguistic experts to validate candidate wake words ([SoundHound](https://voices.soundhound.com/why-voice-assistants-need-to-understand-accents/)).
...
**Custom model creation:**- Collect a broad demographic dataset for your phrase.
...
**Key Metrics:**- **False Acceptance Rate (FAR):**Frequency of incorrect activations (false positives).
- **False Rejection Rate (FRR):**Frequency of missed activations (false negatives).
- **Sensitivity Setting:**Balances FAR and FRR.
- **Latency:**Time from [utterance](/en/glossary/utterance/) to activation.
- **Efficiency:**CPU/memory usage.
- **Robustness:**Performance across noise, accents, and speaker diversity.

**Benchmarking:**Use diverse datasets (different accents, ages, noise types) to evaluate performance. Open datasets and benchmarks are available for this purpose ([Open-source keyword spotting corpora](https://picovoice.ai/blog/open-source-keyword-spotting-data/)).
...
**Solution:**Choose vendors with proven robustness and test with your actual user demographic ([Picovoice Guide](https://picovoice.ai/blog/complete-guide-to-wake-word/)).
...
**Tip:**Use optimized, embedded solutions for maximum battery life ([Porcupine](https://picovoice.ai/platform/porcupine/), [Sensory](https://www.sensory.com/wake-word/)).
...
- **Recommendation:**Avoid cloud-based detection in privacy-sensitive applications ([Picovoice Guide](https://picovoice.ai/blog/complete-guide-to-wake-word/)).

## Implementation Guide

### Step-by-Step: Adding Wake Word Detection

**1. Select a Wake Word Engine**- **Enterprise:**- [Porcupine](https://picovoice.ai/platform/porcupine/)
...
**2. Create Your Custom Wake Word**- Use vendor tools (e.g., [Picovoice Console](https://console.picovoice.ai/)) to define and train your phrase.
  - Collect diverse audio samples for optimal accuracy.

**3. Integrate with Your Application**- Use SDKs for your target platform (iOS, Android, Web, Desktop, Embedded).

**Sample Python Integration:**```python
import pvporcupine
